...
case "$distro" in
  'minlinux' | 'ubuntu' | 'kali' | 'debian' | 'wlinux')
...
case $1 in
  -h | --help)
    ...
    ;;
  -v | --version)
    echo "wslu v$wslu_version; wslsys v$version"
    exit
    ;;
  -I | --sys-installdate)
    printer "Release Install Date: $(call_install_date)" "$2"
    exit
    ;;
  -b | --branch)
    printer "Branch" "$(call_branch)" "$2"
    exit
    ;;
  -B | --build)
    printer "Build" "$(call_build)" "$2"
    exit
    ;;
  -fB | --full-build)
    printer "Full Build" "$(call_full_build)" "$2"
    exit
    ;;
  -U | --uptime)
    ...
    ;;
  -wU | --win-uptime)
    printer "Windows Uptime" "$(call_windows_uptime)" "$2"
    exit
    ;;
  -R | --release)
    printer "WSL Release" "$release" "$2"
    exit
    ;;
  -K | --kernel)
    ...
    ;;
  -P | --package)
    printer "Packages Count" "$packages" "$2"
    exit
    ;;
  -S | --display-scaling)
    printer "Display Scaling" "$(call_display_scaling)" "$2"
    exit
    ;;
  *)
